...
class MainWindow(QMainWindow):
    ...
    def initUI(self):
        ...
        top_sensor_layout.addWidget(self.disp_plot_widget)
        # 오른쪽: 힘(Force) 그래프
        ...
        top_sensor_layout.addWidget(self.force_plot_widget)
        # 가로 뭉치를 메인 레이아웃 가장 위에 추가
        main_layout.addLayout(top_sensor_layout, stretch=2)
       ...
       # 모든 그래프의 X축(시간)을 온도 그래프 기준으로 동기화
        ...
        # 힘/변위 그래프 scale 조정
        self.disp_plot_widget.enableAutoRange(enable=True)
        self.force_plot_widget.enableAutoRange(enable=True)

        # 성능 최적화 (데이터가 쌓여도 버벅이지 않게 함)
